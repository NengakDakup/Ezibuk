				...
				<div className="login-bottom">
					...
					<form onSubmit={(e) => this.formSubmit(e)}>
						...
						<div className="form-group">
							<button
								className={
									email && password && !this.state.loading
										? "btn btn-primary"
										: "btn btn-primary disabled"
								}
								disabled={
									email && password && !this.state.loading ? false : true
								}
							>
								{loading ? (
									<Spinner color="#fff" size={28} width={2} />
								) : (
									"Login"
								)}
							</button>
...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		showBlocker: (payload) => {
			dispatch(showBlocker(payload));
		},
		hideBlocker: (payload) => {
			dispatch(hideBlocker(payload));
		},
	};
};
